...
def pl_true(kb,model):
    # have to write for if iff etc.f
    if kb.symbol[0]:
        return model[kb.symbol]
    elif kb.connective == "and":
        for  child in kb.subexpressions:
            if (pl_true(child, model) == False):
                return false;
        return true;
    elif kb.connective == "or":
        for  child in kb.subexpressions:
            if (pl_true(child, model) == True):
                return true;
        return false;

def extend(p,True,model):
    model[p] = True
    return model
...
def getsym(data):
    ...
    elif data.symbol[0]: # If it is a base case (symbol)
        res.add(data.symbol[0])

    else: # Otherwise it is a subexpression
        for subexpression in data.subexpressions:
            res |= getsym(subexpression)
    return res
...
def tt_entails(kb,a):
    ...
    kbsymbols = getsym(kb)
    asymbols = getsym(a)
    symbols = kbsymbols | asymbols

    return tt_check_all(kb,a,list(symbols),{})
...
def tt_check_all(kb,a,symbols,model):
    ...
    if not symbols:
        ...
        p = symbols.pop(0)
        rest = symbols
        return tt_check_all(kb, a, rest, extend(p,True,model)) and tt_check_all(kb, a, rest, extend(p,False,model))
